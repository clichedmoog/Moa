...
final class ZipCleanerTests: XCTestCase {
    ...
    /// 픽스처의 이름이나 `../` 깊이가 바뀌어도 검증이 저절로 따라간다.
    ///
    /// 픽스처는 `NSTemporaryDirectory()` 바로 아래가 아니라 이 테스트가 직접 만들고
    /// 소유하는 2단계 중첩 디렉터리 안에 둔다. `NSTemporaryDirectory()` 는 언제나
    /// `/var/folders/<hash1>/<hash2>/T/` 형태고, `<hash1>` 은 모든 macOS 설치본에서
    /// `root:wheel drwxr-xr-x` 로 고정돼 있다 — 픽스처를 `T/` 바로 아래 두면 "두 단계
    /// 위로" 탈출하는 시도가 코드가 맞든 틀리든 항상 `EPERM` 으로 막힌다. 그러면 이
    /// 테스트는 구현이 안전해서가 아니라 OS 권한 벽 때문에 늘 통과하는, 실질적으로
    /// 아무것도 검증하지 못하는 테스트가 된다(실측: 리뷰에서 지적됨). 탈출 지점이
    /// 이 테스트가 쓰고 지울 수 있는 디렉터리 안에 오도록 트리를 직접 만들어서,
    /// 통과 여부가 오직 `ZipCleaner` 의 동작으로만 갈리게 한다.
    func testDoesNotWriteOutsideDestination() throws {
        let scratchRoot = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("moa-zipslip-\(getpid())")
        try? FileManager.default.removeItem(at: scratchRoot)
        let nested = scratchRoot.appendingPathComponent("a").appendingPathComponent("b")
        try FileManager.default.createDirectory(at: nested, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: scratchRoot) }

        let localSource = nested.appendingPathComponent("src.zip")
        let localDestination = nested.appendingPathComponent("dst.zip")
        let maliciousName = "../../탈출.txt"
        try makeArchive(entries: [(maliciousName, "악성")], at: localSource)

        _ = try ZipCleaner.clean(archiveAt: localSource, to: localDestination)

        let escapePath = localDestination.deletingLastPathComponent()
    ...
    func testFallsBackToPlaceholderWhenNameIsUndecodable() throws {
        ...
        // 이름만 살고 내용이 사라지면 최악이다 — 이름 없이는 못 찾을 파일이 되어버린다.
        let archive = try Archive(url: destination, accessMode: .read)
        let entry = try XCTUnwrap(archive.first { $0.type == .file })
        var data = Data()
        _ = try archive.extract(entry, skipCRC32: true) { data.append($0) }
    ...
    private func makeArchive(entries: [(String, String)],
        ...
        let archive = try Archive(url: url ?? source, accessMode: .create)
