...
## 10. 미확인 리스크 — 착수 전 검증할 것

두 리스크 모두 샌드박스에서만 발생하며, 하나의 최소 테스트 앱으로 함께 확인한다.
**UI 구현 전에 실측한다.** GUI 드롭이 필요하므로 사용자가 직접 드롭해야 한다.

### 10.1 단일 파일 드롭의 이름 변경이 거부될 가능성

이름 변경은 `old` 경로 삭제 + `new` 경로 생성이라, 샌드박스가 드롭된 파일 **한 개**에만
권한을 내주면 새 이름 경로는 권한 밖이다. 폴더를 드롭하면 하위 트리 전체 권한이 나오므로
안쪽 항목은 괜찮을 것으로 보이나, 폴더 자신의 이름은 같은 문제를 겪는다.

Contact가 같은 entitlement로 동작해온 정황상(9.2) 통과 가능성이 높지만 확정할 수 없다.

### 10.2 ZIP 출력 위치 쓰기가 거부될 가능성

4.5와 4.6은 결과물을 원본 옆(= 원본의 부모 디렉터리)에 만든다.
그러나 샌드박스가 권한을 주는 대상은 **드롭된 항목 자신**이지 그 부모가 아니다.
부모 디렉터리에 새 파일을 만드는 것이 거부될 수 있다.

거부될 경우의 대안은 저장 위치를 `NSSavePanel`로 사용자에게 묻는 것이다
(사용자가 고른 경로에는 권한이 부여된다). 다만 "드롭 한 번으로 끝난다"는 UX가 약해지므로,
실측에서 원본 옆 쓰기가 가능하면 그대로 두고 불가능할 때만 전환한다.

두 검증 결과에 따라 App Store 빌드의 기능 범위를 재논의한다.
