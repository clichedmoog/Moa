...
> 이 문서가 여러 곳에서 정정하는 `HANDOFF.md`는 개발 착수 전에 작성된 인수인계 메모다.
> 내용이 이 스펙으로 대체되어 저장소에서 제거했다 — 원문은 git 이력에 남아 있다
> (`git show bc01f6c:HANDOFF.md`).
...
1. **셸 명령 주입 취약점** — 파일명에서 `"`만 이스케이프하고 `$`, `` ` ``, `\`는 놔둔다.
   큰따옴표 안에서도 `$(...)`와 백틱은 확장되므로 임의 명령이 실행된다.
   우리는 셸을 거치지 않아 문제 자체가 없다.
   **App Store 심사에서도 `/bin/sh` 실행은 거절 사유가 되기 쉬워, 우리 방식이 배포 양쪽 모두에 유리하다.**

   > **실측 (2026-08-17).** Contact 의 스크립트 생성 로직을 그대로 재현해 확인했다.
   > 변환 자체는 정상 동작한다 — `mv` 에 NFC 바이트가 전달되어 디스크가 실제로 바뀐다
   > (`e1 84 92 …` → `ed 95 9c …`). 셸을 거치는 것이 Foundation 의 NFD 변환(3.1)을
   > 피하는 우회로였던 셈이다.
   >
   > 그러나 ``보고서`touch ...`.txt`` 라는 이름의 파일을 드롭하자 백틱 안의 명령이
   > 그대로 실행됐다. 파일명은 공격자가 정할 수 있는 값이므로, 그런 이름의 파일이 든
   > 압축을 받아 드롭하는 것만으로 임의 코드가 실행된다.
   >
   > 이는 선행 앱을 깎아내리려는 기록이 아니다. Contact 는 2020년에 App Store 를
   > 염두에 두지 않고 만들어졌고, 샌드박스 밖이라 `/bin/sh` 를 부를 수 있었다.
   > **우리가 POSIX `rename(2)` 을 직접 부르는 것은 더 나은 판단이라서가 아니라
   > 샌드박스에서 그 길이 애초에 막혀 있었기 때문이고, 그 제약이 결과적으로
   > 주입 문제까지 없앴다.** 제약이 설계를 낫게 만든 사례로 남긴다.
