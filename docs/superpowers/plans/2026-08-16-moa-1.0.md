...
- 생산:
  - `ZipCleaner.clean(archiveAt source: URL, to destination: URL) throws -> ZipResult`
    (`ZipResult` 는 Task 9 에서 정의됨: `entryCount`, `omitted: [ReportEntry]`,
     `failed: [ReportEntry]`, 계산 프로퍼티 `excludedCount`/`failureCount`)
...
3. 둘 다 실패하면 → 원본 이름(`entry.path`) 유지. 추측으로 망가뜨리지 않는다.
   단, 그 원본조차 빈 문자열인 모순된 경우(UTF-8 플래그는 섰는데 원시 바이트가
   실제로는 유효한 UTF-8 이 아닌 zip)에는 빈 이름으로 엔트리를 넣을 수 없으므로
   원시 바이트에서 결정적으로 합성한 이름을 대신 쓴다(리뷰 F4).
...
> **실행 중 변경된 것 (원래 계획 이후):**
> - **샌드박스 entitlements 를 Task 14 에서 앞당겨 여기서 붙였다.** `MoaApp/Moa/Moa.entitlements`
>   (`com.apple.security.app-sandbox`, `com.apple.security.files.user-selected.read-write`)
>   와 `project.yml` 의 `CODE_SIGN_ENTITLEMENTS` 가 이 태스크 커밋에 포함된다. 이유: 이 태스크의
>   수동 검증이 답하려는 유일한 질문 — "샌드박스 아래서 드롭이 실제로 동작하는가" — 는
>   entitlements 없이는 검증 불가능하다. 무해한 상태(unsandboxed)에서 통과한 결과는
>   App Store 빌드에 대해 아무것도 증명하지 못한다.
> - **결과 화면(Task 13)을 Task 12(Dock 드롭) 보다 앞당겨 붙였다.** 최초 수동 드롭에서 하위
>   폴더 안 이름들은 모두 NFC 로 바뀌었는데 드롭한 폴더 자기 자신의 이름은 그대로 NFD 로
>   남는 현상이 나왔다. 그런데 `RootView` 가 `.working` 이후를 전부 `DropView` 로
>   떨어뜨리고 있어 — 완료됐다는 신호 자체가 없어 원인을 알 길이 없었다. Task 13 의
>   `ResultView`/`ConfirmView`/`RootView` 분기 완성을 앞당긴 것은 요청받은 기능이자 동시에
>   이 진단 도구이기도 하다. 자세한 내용과 최종 코드는 아래 **Task 13** 절 참고.
> - `DropCoordinator.CoordinatorState.working` 이 인자 없는 `case` 에서 `case working(String)`
>   (진행 문구)로 바뀌었다. 아래 Step 3 의 `enum CoordinatorState` 코드 블록은 이제
>   실제 코드와 다르다 — Task 13 절의 최신 정의를 따른다.
> - `DropView.swift` 는 이후 애니메이션(호버 버튼 페이드, 드롭 타겟 강조, Reduce Motion 대응)이
>   추가돼 아래 Step 4 코드 블록과 살짝 다르다. 실제 파일 `MoaApp/Moa/DropView.swift` 를 참고한다.
...
> **실행 중 조정된 것:**
> - `ZipResult` 가 이 절 초안 이후 바뀌어 `omitted`/`failed` 를 바레 카운트가 아니라
>   `[ReportEntry]` 로 들고 있다(`excludedCount`/`failureCount` 는 계산 프로퍼티). 아래
>   `RootView` 의 `.zipped` 분기는 `omitted` 뿐 아니라 `failed` 도 이름 있는 목록으로
>   보여주도록 고쳤다 — 원안은 `failed` 를 `failureCount` 로만 보여줬는데, 그러면
>   ".DS_Store 하나 제외" 와 "번들 통째로 못 읽음" 이 똑같이 "숫자 하나"로 뭉개진다.
> - 실패 사유(`entry.detail`) 는 어디서도 자르거나 다듬지 않는다. `NormalizationService`
>   가 흔한 errno 는 한글로, 나머지는 `실패 (errno N)` 로 떨어뜨리는데 그 fallback
>   문구까지 그대로 보여준다 — 지금은 이게 원인을 알 유일한 방법이다.
> - "실패" 를 스크롤 목록 안에만 두지 않는다. `ResultView`/`.zipped` 화면 모두 스크롤
>   밖의 상시 노출 배너로 먼저 알리고, 목록 안에서도 옅은 붉은 배경으로 다시 강조한다 —
>   9개를 바꾸고 1개를 실패해도 "성공"으로만 읽히면 안 된다.
> - `RootView` 전체를 `Group` + `switch` + `.transition(.opacity)` + `.animation(value:)`
>   로 감싸 상태 전환(드롭 → 진행 중 → 결과)이 크로스페이드되게 했고, `.working` 은
>   빈 스피너 대신 `"N개 항목의 이름을 바꾸는 중…"` 처럼 무엇이 처리되는지 말한다
>   (`DropCoordinator.CoordinatorState.working` 이 `case working(String)` 으로 바뀜).
>   모든 애니메이션은 `@Environment(\.accessibilityReduceMotion)` 으로 게이트해
>   Reduce Motion 이 켜져 있으면 즉시 전환된다.
> - `.zipped`/`.failed` 분기는 `RootView` 안에 인라인으로 두지 않고 `ZippedView`/
>   `FailedView` 라는 파일 전용(`private`) 뷰로 뺐다. 표시 내용은 같다.
...
struct RootView: View {
    ...
    var body: some View {
        ...
        // 유틸리티 앱이라 과하지 않게, Reduce Motion 이 켜져 있으면 즉시 바뀐다.
        .animation(reduceMotion ? nil : .easeInOut(duration: 0.2), value: stateTag)
    }
}

/// `.zipped` 상태 화면. `omitted` 는 흔한 일(`.DS_Store` 등)이라 접어두지만,
/// `failed` (읽지 못해 아예 빠진 항목)는 절대 숨기지 않는다 — 모른 채 USB 로
/// 옮기면 파일이 사라진 줄도 모른다.
private struct ZippedView: View {
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            // `omitted` 에는 맥 전용 파일뿐 아니라 심볼릭 링크·번들·특수 파일도 들어온다.
            // 문구를 "맥 전용 파일"로 좁히면 거짓말이 된다.
...
`MoaApp.swift` 파일 맨 위 import 에 `MoaKit` 이 추가됐다 — `ZippedView` 가
`ZipResult` 를 명시적 타입으로 쓰기 때문이다 (`import AppKit`, `import MoaKit`,
`import SwiftUI`).
