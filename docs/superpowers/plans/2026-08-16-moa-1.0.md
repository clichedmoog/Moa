...
final class DropCoordinator: ObservableObject {
    ...
    private func cleanArchive(at url: URL) {
        let destination = ZipCleaner.defaultDestination(for: url)
        state = .working
        Task {
...
> **Task 5 결과 반영:** 샌드박스 실측에서 "부모 디렉터리 쓰기"가 **실패**했다면
> `ZipCleaner` 의 출력도 위와 같이 `NSSavePanel` 로 위치를 물어야 한다.
> `DropCoordinator.cleanArchive(at:)` 에서 `ZipCleaner.defaultDestination(for:)` 대신
> 저장 패널 결과를 쓰도록 바꾼다. 실측이 성공이면 지금 코드 그대로 둔다.
