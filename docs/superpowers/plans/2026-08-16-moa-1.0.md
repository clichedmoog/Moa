...
Xcode에서 macOS App 템플릿으로 생성한다.

| 설정 | 값 |
| --- | --- |
| Product Name | Moa |
| Team | (본인 Apple 계정) |
| Organization Identifier | com.clichedmoog |
| Bundle Identifier | com.clichedmoog.Moa |
| Interface | SwiftUI |
| Language | Swift |
| 저장 위치 | `MoaApp/` |
...
- **Build Settings → Architectures** → `$(ARCHS_STANDARD)` (arm64 + x86_64 확인)
- **File → Add Package Dependencies → Add Local** → 저장소 루트 선택 → `MoaKit` 추가

- [ ] **Step 2: 빌드가 통과하는지 확인**

실행: `xcodebuild -project MoaApp/Moa.xcodeproj -scheme Moa -configuration Debug build`
예상: BUILD SUCCEEDED
...
- [ ] **Step 1: Info.plist에 문서 타입 등록**

`MoaApp/Moa/Info.plist` 에 추가:
...
Xcode에서 **Project → Info → Configurations** 에 연결한다.
Release 구성을 복제해 `Release-AppStore` 를 만들고 각각 해당 xcconfig 를 지정한다.
